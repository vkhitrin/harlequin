...
class HarlequinDebugInfo:
    ...
    def details(self) -> list[tuple[str, object]]:
        try:
...
class AdapterDebugInfo:
    ...
    def details(self) -> list[tuple[str, object]]:
        type_md = f"### Type\n`{self.adapter_type}`" if self.adapter_type else ""
        details_md = f"`{self.adapter_details}`"
        if self.adapter_options:
            ...
            options_content = "No adapter options defined."
        return [
            (
                "Adapter Details",
                [
                    ("Type", type_md),
                    ("Details", details_md),
                    ("Adapter Options", options_content),
                ],
            ),
        ]
...
class DebugInfoScreen(ModalScreen):
    def __init__(
        self,
        harlequin_details: list[tuple[str, str]],
        adapter_details: list[tuple[str, str]],
        name: str | None = None,
        id: str | None = None,  # noqa: A002
        classes: str | None = None,
    ) -> None:
    ...
    def on_mount(self) -> None:
        ...
        # Set focus to the first collapsible
        first = self.query_one("#collapsible-harlequin-details")
